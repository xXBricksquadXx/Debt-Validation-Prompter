...
const buildPolishLetterMessages = (d) => {
  const system = [
    ...
    "Use only these tags: h3, div, p, b, br, ul, ol, li.",
    "Do not include scripts/styles/iframes.",
    "CRITICAL: Do NOT invent bracketed placeholders like [YOUR FULL NAME].",
    "Only include bracketed placeholders if the exact placeholder text already appears in the provided input fields.",
    "If a field has a real value, use it exactly and do not show any placeholder variant.",
    "CRITICAL: Mirror the provided TEMPLATE STRUCTURE exactly (same headings/sections order).",
  ].join(" ");
  ...
  const template = [
    ...
    "  {{#if JURISDICTION}}<b>Jurisdiction:</b> {{JURISDICTION}}<br/>{{/if}}",
    "  {{#if STATUTES}}<b>References:</b> {{STATUTES}}{{/if}}",
    "</div>",
    "",
    '<div class="box">',
    ...
    '  <div class="hr"></div>',
    "  <div>",
    '    <div class="tiny"><b>From</b></div>',
    "    <b>{{CONSUMER_NAME}}</b><br/>",
    "    {{CONSUMER_ADDRESS}}",
    '    {{#if CONSUMER_EMAIL}}<br/><div class="tiny">{{CONSUMER_EMAIL}}</div>{{/if}}',
    '    {{#if CONSUMER_PHONE}}<div class="tiny">{{CONSUMER_PHONE}}</div>{{/if}}',
    "  </div>",
    "  <br/>",
    "  <div>",
    '    <div class="tiny"><b>To</b></div>',
    "    {{COLLECTOR_NAME}}<br/>",
    "    {{COLLECTOR_ADDRESS}}",
    '    {{#if COLLECTOR_EMAIL}}<br/><div class="tiny">Email: {{COLLECTOR_EMAIL}}</div>{{/if}}',
    "  </div>",
    "</div>",
    "",
    "<p>To whom it may concern:</p>",
    "<p>I am writing regarding the alleged debt referenced above. I dispute the validity of this debt and request validation and supporting documentation. If you cannot validate, do not continue collection activity and confirm in writing.</p>",
    "",
    '<div class="box"><b>Snapshot (facts only):</b><br/>{{SNAPSHOT_LINES}}</div>',
    "",
    '{{#if SUMMARY}}<div class="box"><b>Situation summary:</b><br/>{{SUMMARY}}</div>{{/if}}',
    "",
    '<div class="box"><b>Requested validation / documentation:</b>{{REQUESTS_OL}}</div>',
    "",
    '{{#if COMM_PREFS}}<div class="box"><b>Communication preferences:</b>{{COMM_PREFS_UL}}</div>{{/if}}',
    "",
    "<p>Please provide your response in writing. If you have reported or will report information regarding this alleged debt to any consumer reporting agency, ensure your reporting is accurate and consistent with your validation.</p>",
    "<p>Sincerely,<br/><b>{{CONSUMER_NAME}}</b></p>",
  ].join("\n");

  const user = [
    "Fill the TEMPLATE STRUCTURE exactly. Do not add/remove sections.",
    "Rules:",
    "- Use <br/> for line breaks in addresses and snapshot lines.",
    "- Snapshot lines must be 5–10 lines, facts only, each separated by <br/>.",
    "- Requests must be an <ol> list. Communication prefs must be a <ul> list.",
    "- If a value is missing, omit that specific line/field entirely (do not insert 'Not provided').",
    "",
    "TEMPLATE STRUCTURE:",
    template,
    "",
